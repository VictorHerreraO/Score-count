 ...
 * @param onDismiss Callback invoked when the user requests to dismiss the sheet (e.g., by tapping the scrim or back button).
 ...
 * @param onOptionSelected Callback invoked with the selected option [T] when the user taps an item. The sheet will automatically attempt to hide after this is called.
 * @param sheetState The state of the modal bottom sheet, which can be used to control it programmatically.
 * @param optionContent A composable lambda that defines the content for each individual option. It's invoked for each item in the [options] list, receiving the item `T` as a parameter. The entire area of this content will be clickable.
 */
